(function(mod) {
  if (typeof exports == "object" && typeof module == "object") { // CommonJS
    return mod(require("tern/lib/infer"), require("tern/lib/tern"), require("jscs/lib/string-checker"), require("jscs/lib/cli-config"), require('path'));
  }
  if (typeof define == "function" && define.amd) // AMD
    return define([ "tern/lib/infer", "tern/lib/tern", "jscs/lib/string-checker", "jscs/lib/cli-config", "path" ], mod);
  mod(tern, tern, JscsStringChecker, configFile, path);
})(function(infer, tern, StringChecker, configFile, path) {
  "use strict";
  ...
  function loadConfig(customFile, file) {
    var config;
    if (customFile.configFile == "default") {
      config = configFile.load(null, path.dirname(file));
    } else if (customFile.configFile) {
      config = configFile.load(customFile.configFile);
    }
  ...
  function validate(server, query, file, messages) {
    ...
    var options = loadConfig(server.mod.jscs, path.join(server.projectDir, file.name));
