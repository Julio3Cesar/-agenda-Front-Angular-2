(function(mod) {
  if (typeof exports == "object" && typeof module == "object") { // CommonJS
    return mod(require("tern/lib/infer"), require("tern/lib/tern"), require("eslint/lib/eslint"), require("path"));
  }
  if (typeof define == "function" && define.amd) // AMD
    return define([ "tern/lib/infer", "tern/lib/tern", "eslint/lib/eslint", "path" ], mod);
  mod(tern, tern, eslint, path);
})(function(infer, tern, eslint, path) {
  "use strict";
  ...
  function ESLintConfig(server, options) {
    if (options.configFile == "default") {
      this.defaultConfig = true;
    } else if (options.config && !isEmpty(options.config)) {
  ...
  ESLintConfig.prototype.update = function(file) {
    if (this.defaultConfig) {
      var Config = require("eslint/lib/config");
      this.config = new Config({useEslintrc : true, cwd: process.cwd()}).getConfig(file);
      return;
    }
  ...
  function validate(server, query, file, messages) {
	...
	eslintConfig.update(path.join(server.projectDir, file.name));
