...
var TSLintEndpoint = (function () {
        ...
        if (fsName && linter) {
            ...
            if (modifiedFiles === null || modifiedFiles === undefined
                ...
                program.getSourceFiles()
                    .filter(function (f) { return f.fileName.endsWith(".ts")
                    && !f.fileName.endsWith(".d.ts")
                    && (f.fileName.indexOf("/node_modules/") < 0); })
                ...
                modifiedFiles
                    ...
                    var f = program.getSourceFile(name);
                    if (f) {
                        toCheck_1.add(f);
                    }
                    else {
                ...
                try {
                    ...
                    if (lintRes) {
                        lintRes.failures.forEach(function (failure) {
                            ...
                            fileRes.push(new services_1.FileMarker(services_1.MarkerType.TSLINT_FAILURE, start, end, failure.getStartPosition().getLineAndCharacter().line + 1, failure.getFailure(), services_1.MarkerPriority.PRIORITY_NORMAL, severity, failure.getRuleName(), quickFixes));
                        });
                    }
                }
                catch (e) {
                    if (e.message) {
                        ...
                        fileRes.push(new services_1.FileMarker(services_1.MarkerType.TSLINT_FAILURE, 0, lineStarts.length > 1 ? lineStarts[1] : f.text.length, 1, "Internal error in " + linter.name() + ": " + e.message, services_1.MarkerPriority.PRIORITY_NORMAL, services_1.MarkerSeverity.SEVERITY_WARNING));
    ...
    TSLintEndpoint.prototype.createLinter = function (projectName, tsLintVersion) {
        ...
        if (tsLintVersion && tsLintVersion !== "default") {
            embedded = true;
            tsLintModule = path.join(__dirname, tsLintVersion === "5.2.0"
                ? "../../node_modules/tslint"
    ...
    TSLintEndpoint.prototype.createQuickFixes = function (projectName, quickFixRequests, formattingOptions) {
        ...
        if (fsName && linter) {
            ...
            quickFixRequests.forEach(function (req) {
                (reqsPerFile[req.fileName] || (reqsPerFile[req.fileName] = [])).push(req);
            });
            var _loop_1 = function () {
                ...
                if (lintRes) {
                    var reqsMap_1 = {};
                    for (var _i = 0, _a = reqsPerFile[fileName]; _i < _a.length; _i++) {
                        var req = _a[_i];
                        reqsMap_1[req.start] = {
                            start: req.start,
                            end: req.end,
                    ...
                    lintRes.failures.forEach(function (failure) {
                        ...
                        var sameTypeReqs = [];
                        if (reqsPerFile[fileName].length > 1) {
            ...
            for (var _i = 0, _b = Object.keys(reqsPerFile); _i < _b.length; _i++) {
                var fileName = _b[_i];
                _loop_1();
            }
        }
