...
var LookupModuleScriptsProvider = (function () {
    ...
    LookupModuleScriptsProvider.prototype.gatherModuleScripts = function (module) {
        ...
        if (snapshot !== undefined && snapshot !== null) {
            ...
            if (!typingsFile) {
                var main_2 = modulePackageJson["main"];
                if (main_2 && main_2.endsWith(".js")) {
                    typingsFile = main_2.substring(0, main_2.length - 2) + "d.ts";
                    moduleImport = path.posix.dirname(main_2);
                    var baseName = path.posix.basename(main_2);
                    if (baseName !== "index") {
...
function createQuickFixesForImport(file, program, missingName, start, end, importInfo, result) {
    ...
    function handleNewImport(importName) {
        ...
        var newText = "import { " + name + " } from '" + importName + "';\n";
        var changes;
        var msgText;
        if (name !== missingName) {
