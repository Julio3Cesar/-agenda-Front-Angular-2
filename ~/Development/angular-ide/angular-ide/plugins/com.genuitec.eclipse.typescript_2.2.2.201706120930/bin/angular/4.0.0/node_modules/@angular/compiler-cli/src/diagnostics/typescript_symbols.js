...
function getSymbolQuery(program, checker, source, fetchPipes, langService) {
    return new TypeScriptSymbolQuery(program, checker, source, fetchPipes, langService);
}
exports.getSymbolQuery = getSymbolQuery;
function getClassMembers(program, checker, staticSymbol, langService) {
    ...
    if (declaration) {
        ...
        return new TypeWrapper(type, { node: node, program: program, checker: checker, langService: langService }).members();
    }
}
exports.getClassMembers = getClassMembers;
function getClassMembersFromDeclaration(program, checker, source, declaration, langService) {
    var type = checker.getTypeAtLocation(declaration);
    return new TypeWrapper(type, { node: source, program: program, checker: checker, langService: langService }).members();
}
...
function getPipesTable(source, program, checker, pipes, langService) {
    return new PipesTable(pipes, { program: program, checker: checker, node: source, langService: langService });
}
exports.getPipesTable = getPipesTable;
var TypeScriptSymbolQuery = (function () {
    function TypeScriptSymbolQuery(program, checker, source, fetchPipes, langService) {
    ...
    TypeScriptSymbolQuery.prototype.getBuiltinType = function (kind) {
        ...
        if (!result) {
            var type = getBuiltinTypeFromTs(kind, { checker: this.checker, node: this.source, program: this.program, langService: this.langService });
            result =
                new TypeWrapper(type, { program: this.program, checker: this.checker, node: this.source, langService: this.langService });
    ...
    TypeScriptSymbolQuery.prototype.getTemplateContext = function (type) {
        var context = { node: this.source, program: this.program, checker: this.checker, langService: this.langService };
...
var TypeWrapper = (function () {
    ...
    Object.defineProperty(TypeWrapper.prototype, "documentation", {
        get: function () { return documentationFromTsSymbol(this.tsType.getSymbol(), this.context, this); },
        enumerable: true,
        configurable: true
    });
...
var SymbolWrapper = (function () {
    ...
    Object.defineProperty(SymbolWrapper.prototype, "documentation", {
        get: function () {
            var result = documentationFromTsSymbol(this.symbol, this.context, this);
            if (result && result.documentation.length === 0
                && (this.symbol.parent)) {
                //try to get documentation from parent
                var parentDoc = documentationFromTsSymbol(this.symbol.parent, this.context, this);
                if (parentDoc && parentDoc.documentation && parentDoc.documentation.length > 0) {
                    result.documentation = parentDoc.documentation;
                }
            }
            return result;
        },
...
var DeclaredSymbol = (function () {
    ...
    Object.defineProperty(DeclaredSymbol.prototype, "documentation", {
        get: function () { return this.declaration.documentation; },
        enumerable: true,
        configurable: true
    });
...
var PipeSymbol = (function () {
    ...
    Object.defineProperty(PipeSymbol.prototype, "documentation", {
        get: function () {
            //Get documentation on the pipe class if possible
            var parent = this.tsType.getSymbol().parent;
            var doc = documentationFromTsSymbol(parent ? parent : this.tsType.getSymbol(), this.context, this);
            if (doc) {
                //change kind and keyword
                doc.kind = "pipe";
                doc.displayParts[0].text = "pipe";
            }
            return doc;
        },
        enumerable: true,
...
function findClassSymbolInContext(type, context) {
    ...
    if (sourceFile) {
        ...
        var result = (exports_1 || []).find(function (symbol) { return symbol.name == type.name; });
        if (result && (result.getFlags() & ts.SymbolFlags.Alias) !== 0) {
            result = context.checker.getAliasedSymbol(result);
        }
        return result;
    }
}
...
function documentationFromTsSymbol(symbol, context, ngSymbol) {
    var kind;
    var kindModifiers;
    var displayParts;
    ...
    if (docs && docs.length > 0) {
        var d = docs[0];
        documentation = [{ kind: "jsdoc", text: source.text.substring(d.pos, d.end) }];
        tags = [];
    }
    if (!documentation) {
    ...
    var pos = declaration.getStart();
    if (declaration.name && declaration.name.getStart() >= 0) {
        pos = declaration.name.getStart();
    }
    var info = context.langService ? context.langService.getQuickInfoAtPosition(source.fileName, pos) : null;
    if (info) {
        kind = info.kind;
        ...
        displayParts = [
            { kind: "text", text: "(" + kind + ") " },
            { kind: "className", text: ngSymbol.name }
        ];
        var container = ngSymbol.container;
        if (container) {
            displayParts.push({ kind: "text", text: ' of ' }, { kind: "className", text: container.name });
        }
    }
    return {
        span: { start: -1, end: -1 },
        displayParts: displayParts, kind: kind, kindModifiers: kindModifiers,
...
function typeKindOf(type) {
            ...
            if (unionType.types.length > 0) {
                ...
                for (var _i = 0, _a = unionType.types; _i < _a.length; _i++) {
                    ...
                    if (candidate != typeKindOf(subType)) {
                        return type.symbol ? symbols_1.BuiltinType.Other : symbols_1.BuiltinType.Any;
                    }
                }
            }
            if (candidate != null) {
    ...
    return (type && type.symbol) ? symbols_1.BuiltinType.Other : symbols_1.BuiltinType.Any;
}
