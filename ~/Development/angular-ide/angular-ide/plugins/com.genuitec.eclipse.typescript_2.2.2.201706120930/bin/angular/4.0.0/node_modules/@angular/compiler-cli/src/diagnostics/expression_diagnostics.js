...
var completions_1 = require("../../../language-service/src/completions");
var html_info_1 = require("../../../language-service/src/html_info");
var compiler_1 = require("@angular/compiler");
var expression_type_1 = require("./expression_type");
var symbols_1 = require("./symbols");
function getTemplateExpressionDiagnostics(info, info2) {
    var visitor = new ExpressionDiagnosticsVisitor(info, function (path, includeEvent) {
        ...
        return completions_1.getAttributeInfosForElement(info2, elementName, element);
    });
    ...
    function processReferences(references) {
        var _loop_1 = function (reference) {
            ...
            result.push({
                ...
                get definition() { return getDefintionOf(info, reference, "reference", reference.name); },
                get documentation() { return getDocumentationOf(info, reference, "reference", reference.name); }
            });
        };
...
function getDefintionOf(info, ast, kind, name) {
    if (info.fileName) {
...
function getDocumentationOf(info, ast, kind, name) {
    if (info.fileName) {
        var templateOffset = info.offset;
        return {
            displayParts: [{
        ...
        if (current instanceof compiler_1.EmbeddedTemplateAst) {
            var _loop_2 = function (variable) {
                ...
                result.push({
                    ...
                    kind: 'variable',
                    type: type,
                    get definition() { return getDefintionOf(info, variable, 'var', name_1); },
                    get documentation() { return getDocumentationOf(info, variable, 'var', name_1); }
                });
            };
...
var ExpressionDiagnosticsVisitor = (function (_super) {
    ...
    function ExpressionDiagnosticsVisitor(info, getExpressionScope, getAttributeInfosForElement) {
        ...
        _this.getAttributeInfosForElement = getAttributeInfosForElement;
        _this.diagnostics = [];
    ...
    ExpressionDiagnosticsVisitor.prototype.visitAttr = function (ast) {
        ...
        if (!(element instanceof compiler_1.ElementAst) || !element.name) {
            return;
        }
    ...
    ExpressionDiagnosticsVisitor.prototype.reportError = function (message, span) {
        if (span) {
            this.diagnostics.push({ span: offsetSpan(span, this.info.offset), kind: expression_type_1.DiagnosticKind.Error, message: message, type: "expression-problem" });
        }
    };
    ExpressionDiagnosticsVisitor.prototype.reportWarning = function (message, span) {
        this.diagnostics.push({ span: offsetSpan(span, this.info.offset), kind: expression_type_1.DiagnosticKind.Warning, message: message, type: "expression-problem" });
    };
